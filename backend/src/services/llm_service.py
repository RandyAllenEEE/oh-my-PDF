...
class LLMService:
    ...
    def call_vlm(self, input_path: Path, start_page: int, end_page: int, config: Dict[str, Any]) -> str:
        """
        ...
        prompt = config.get('prompt', 'Extract the table of contents from these images. Return the results in Title @ Page format.')
        ...
        if provider == 'ollama':
            ...
            if "/v1/v1" in url: url = url.replace("/v1/v1", "/v1") # simple fix for double v1
        ...
        logger.info(f"Calling VLM ({provider}) at {url} with model {model} for pages {start_page}-{end_page}")
    ...
    def call_llm(self, text: str, config: Dict[str, Any]) -> str:
        ...
        prompt_template = config.get('prompt', 'Fix the following Table of Contents text:\n\n{text}')
